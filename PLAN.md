...
연구실 데스크탑(Ubuntu 24.04, 학교 내부망)에서 `http://<데스크탑 IP>:8000` 하나로
...
| 타임라인 표시 | **14일 전체를 한 번에 그리고 가로 스크롤**, GPU 이름 열은 왼쪽 고정 |
| 관리자 예약 수정 범위 | **시작·종료 시각만 수정** + 삭제. GPU·예약자 변경 불가.<br>**중복 검사는 항상 적용**, 시간 길이(min/max)·14일 범위 제한은 **관리자만 무시 가능** |
...
├── frontend/
│   ├── package.json
│   ├── vite.config.js           # 개발 중 /api 요청을 8000 포트로 넘겨주는 proxy 설정
│   ├── index.html
│   └── src/
│       ├── main.js, App.vue
...
rules:
  ...
  booking_horizon_days: 14    # 오늘부터 며칠 뒤까지 예약 가능
  short:                      # 단주기
    min_hours: 1
    max_hours: 48
  long:                       # 장주기
...
| PATCH | `/api/admin/reservations/{id}` | **시작·종료 시각만** 수정. 중복 검사는 항상, 길이·14일 제한은 무시 가능 | 관리자 |
...
함수는 `as_admin: bool = False` 인자를 받아, 관리자가 부를 때만 일부 검사를 건너뛴다.
...
- [ ] `services/reservation_rules.py` — 위 6단계 검사
- [ ] `routers/reservations.py` — 생성(트랜잭션) / 조회 / 내 예약 / 취소 / 조기 종료
- [ ] pytest: 겹침 거부, 경계 맞닿음 허용, 단주기·장주기 시간 규칙, 가입코드 오류,
      **현재가 14:20일 때 14:00 시작은 허용 / 13:00 시작은 거부** (시간을 고정해서 테스트)

**완료 확인:** `pytest` 전부 통과 + `uvicorn` 실행 후 `http://localhost:8000/docs`에서 직접 호출해보기
...
**1) 작업 목록**
- [ ] `TimelineView` + `TimelineGrid` — 가로: 오늘 0시~14일 후 시간 칸 336개 / 세로: GPU 12행
...
- [ ] `AdminView` — 전체 예약 목록, 시간 수정 / 삭제. 규칙 무시 저장 시 경고 문구 표시
- [ ] `routers/admin.py` — PATCH(시간만, `as_admin=True`) / DELETE, 관리자 검사
...
- [ ] pytest:
      ...
      - **관리자 PATCH는 단주기에 100시간, 20일 뒤 시작 등 길이·14일 제한을 무시하고 저장 성공**
      - **일반 사용자에게는 같은 요청이 400으로 거부** (규칙이 관리자에게만 완화되는지 확인)
      - 수정 시 자기 자신과는 겹침 판정을 하지 않음

**완료 확인:** 일반 계정으로 남의 예약 취소 시도 → 거부되는 테스트 통과,
그리고 관리자 규칙 완화·중복 금지 테스트 통과
...
uvicorn app.main:app --host 0.0.0.0 --port 8000   # http://<IP>:8000 하나로 전부
```
